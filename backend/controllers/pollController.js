...
exports.getAllPolls = async (req, res) => {
  ...
  try {
    ...
    //Add `userHasPoll` flag for each poll
...
exports.getVotedPolls = async (req, res) => {
  ...
  try {
    ...
    //Get total count of voted polls for pagination exists
    const totalVotedPolls = await Poll.countDocuments({ voters: userId });
    console.log("Total Voted Polls:", totalVotedPolls);

    //fetch polls where the user has voted
    const polls = await Poll.find({ voters: userId }) //Filter by polls where the user's ID exists
    ...
    //Addd `userHasVoted` flag for eeach poll
    const updatedPolls = polls.map((poll) => {
      const userHasVoted = poll.voters.some((voterId) =>
        voterId.equals(userId)
      );
    ...
    console.log("Error fetching voted polls:", err.message);
    res
      .status(500)
      .json({ message: "Error fetching voted polls", error: err.message });
  }
};
...
exports.getPollById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching poll by user", error: err.message });
  }
};
...
exports.closePoll = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Poll closed succesfully", poll });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Error in closing poll", error: err.message });
  }
};
