...
import CharAvatar from "../../components/cards/CharAvator";

const StatsInfo = ({ label, value }) => {
  return (
    <div className="text-center">
      <p className="font-medium text-gray-950">{value}</p>
      <p className="text-xs text-slate-700/80 mt-[12px]">{label}</p>
    </div>
  );
};
  ...
  return (
    <div className="bg-slate-100/50 rounded-lg mt-16 overflow-hidden">
      <div className="w-full h-32 bg-profile-bg--img bg-cover flex justify-center bg-sky-500 relative">
        <div className="absolute -bottom-10 rounded-full overflow-hidden border-2 border-cyan-500">
          {profileImageUrl ? (
            <img
            src={profileImageUrl || ""}
            alt="Profile Image"
